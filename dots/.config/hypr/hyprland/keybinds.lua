...
hl.bind("CTRL + SUPER + R", hl.dsp.exec_cmd("killall ags agsv1 gjs ydotool qs quickshell"),
...
local function zoomscale(factor)
    local zoomvalue = hl.get_config("cursor:zoom_factor")
    local nextvalue = zoomvalue * factor
    if nextvalue > 3.0 then nextvalue = 3.0 end
    if nextvalue < 1.0 then nextvalue = 1.0 end
    hl.config({ cursor = { zoom_factor = nextvalue } })
end
hl.bind("SUPER + Minus", function() zoomscale(0.8) end, { repeating = true, description = "Screen: Zoom out" })
hl.bind("SUPER + Equal", function() zoomscale(1.25) end, { repeating = true, description = "Screen: Zoom in" })
hl.bind("SUPER + Backspace", function() hl.config({ cursor = { zoom_factor = 1.0 } }) end)
